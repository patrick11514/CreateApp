...
export const jwt = new JWTCookies(JWT_SECRET)\n`);
        ...
        if (tools.includes('mysql')) {
            ...
            const data = await request.text();
            const serverPath = node_path_1.default.join(path, 'src', 'lib', 'server');
            if (!node_fs_1.default.existsSync(node_path_1.default.join(serverPath, 'mysql'))) {
                node_fs_1.default.mkdirSync(node_path_1.default.join(serverPath, 'mysql'));
            }
            node_fs_1.default.writeFileSync(node_path_1.default.join(serverPath, 'mysql', 'main.ts'), data);
            node_fs_1.default.appendFileSync(node_path_1.default.join(serverPath, 'variables.ts'), `import {
    DATABASE_IP,
    DATABASE_PASSWORD,
    DATABASE_PORT,
    DATABASE_USER
...
\`\`\``);
        ...
        node_fs_1.default.writeFileSync(node_path_1.default.join(path, 'svelte.config.js'), svelteJS.toString().replace(/@sveltejs\/adapter-auto/g, `@sveltejs/adapter-${adapter}`));
